-- Создаем БД
DROP DATABASE IF EXISTS pt_dasql_sem4; -- Удвляем БД, если она существует
CREATE DATABASE IF NOT EXISTS pt_dasql_sem4; -- Создаем БД, если её не было

-- Выбираем созданную БД
USE pt_dasql_sem4;

-- Создадим и заполним таблицу (users)
CREATE TABLE users
( 
...
-- Создадим и заполним таблицу (clients)
...
-- Задание на EXISTS
-- Создадим и заполним таблицу (Employee)
...
-- Создадим и заполним таблицу (Projects)
...
-- Собеседования
-- Создадим таблицы (AUTO, CITY, MAN)
CREATE TABLE  AUTO 
(       
...
-- Заполним таблицы (AUTO, CITY, MAN)
-- AUTO
...
SELECT a.MARK, COUNT(*) AS COUNT_NOT_MARK
  FROM AUTO a
  LEFT JOIN AUTO b 
    ON a.MARK <> b.MARK AND a.REGNUM <> b.REGNUM
GROUP BY a.MARK;

-- Задание № 3. Даны 2 таблицы, созданные следующим образом:
CREATE TABLE  test_a (id INT, data_test VARCHAR(1));
CREATE TABLE  test_b (id INT);

INSERT INTO test_a(id, data_test) VALUES
(10, 'A'),
(20, 'A'),
(30, 'F'),
(40, 'D'),
